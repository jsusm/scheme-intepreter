...
export type NoteBookEditorAction =
	...
	| { type: 'setOutput', payload: { output: string[], error: string, idx: number } }
...
function NoteBookEditorReducer(state: NotebookEditorState, action: NoteBookEditorAction): NotebookEditorState {
	switch (action.type) {
		case "addBlock":
			return {
				...state,
		...
		case "removeBlock":
			return { ...state, codeBlocks: state.codeBlocks.filter((_, idx) => idx != action.payload.idx) }
		case "changeCode":
			return {
				...state,
				codeBlocks:
					state.codeBlocks.map((c, idx) => (idx == action.payload.idx ? { ...c, code: action.payload.code } : c)),
			}
		case "setOutput":
			return {
				...state,
				codeBlocks:
					state.codeBlocks.map((c, idx) => (idx == action.payload.idx ? { ...c, error: action.payload.error, output: action.payload.output, executionOrder: state.lastExecution + 1 } : c)),
		...
		case "hideOuput":
			return {
				...state,
				codeBlocks:
					state.codeBlocks.map((c, idx) => (idx == action.payload.idx ? { ...c, hide: true } : c)),
			}
		case "showOutput":
			return {
				...state,
				codeBlocks:
					state.codeBlocks.map((c, idx) => (idx == action.payload.idx ? { ...c, hide: false } : c)),
			}
	}
}
...
function getNoteBookStateFromLS() {
	...
	lastState.codeBlocks = lastState.codeBlocks.map(v => ({ ...v, executionOrder: -1, output: [], error: '' }))
	return lastState
}
...
export function useNoteBookState() {
	const [noteBookEditorState, noteBookEditorDispatch] = useReducer(NoteBookEditorReducer, getNoteBookStateFromLS() ?? {
		lastExecution: 0, codeBlocks: [{
			code: '',
			error: '',
			executionOrder: -1,
			hide: false,
