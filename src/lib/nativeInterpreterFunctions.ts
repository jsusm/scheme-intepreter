...
import { isTruthyValue, primitiveValueToString, type ListValue, type PrimitiveValue } from "./InterpreterDataTypes"
import type { NumberNode, SymbolNode } from "./ParserNodeTypes"

export type NativeFunction = (params: PrimitiveValue[], environment: Environment, output: string[]) => PrimitiveValue
...
export class NativeFunctionRepository {
  ...
  addFunction(label: string, paramsCount: number | number[], fn: NativeFunction) {
  ...
  executeNativeFunction(fnName: string, params: PrimitiveValue[], env: Environment, output: string[]): PrimitiveValue {
...
const list: NativeFunction = (params, _env, _output) => {
  ...
  const list: ListValue = { type: 'list', car: params[0], cdr: { type: 'null' } }
  let curr = list;
  for (let i = 1; i < params.length; i++) {
